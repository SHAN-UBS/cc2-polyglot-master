...
public class HappySevenTest {
	...
	private Info min14 = new Info(Integer.MAX_VALUE);
	...
	public void testAllPermutations() throws Exception {
		...
		System.out.printf("min 14  : %s -> %s%n", min14.in, min14.ops);
	...
	private void record(final List<Integer> configuration, final Bindings result) {
		...
		if (ops.size() > 13 && min14.size > ops.size()) {
			min14.size = ops.size();
			min14.ops = ops;
			min14.in = configuration;
		}
